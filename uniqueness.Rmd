...
calc_uniqueness = function(tib, sim_method, handle_missing_values=T){
  ...
  stopifnot(sim_method %in% c('cosine','euclidean'))
  ...
  if (sim_method == 'euclidean'){
    # calculate cosine similarities between rows
    edist = dist(indata, diag=T, upper=T) %>% as.matrix() %>% as_tibble()
    stopifnot(edist >= 0)
...
# synthetic data
res = tibble()
for (method in c('cosine','euclidean'))
for (trial in seq(10))
for (row_n in seq(0,1000,200)){
  for (col_n in seq(0,100,10)){

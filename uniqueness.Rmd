...
pacman::p_load(tidyverse, moments, knitr, corrr, entropy, pastecs, coop, sf, 
               outliers, mvoutlier, flextable, rstatix, FSA, tmap) # mahalanobis
...
## London boroughs
TODO: 
Boroughs of Greater London with socio-economic variables.
...
lnd_boro_u = dist_uniq(lnd_boro %>% st_drop_geometry(), 'euclidean')
...
openxlsx::write.xlsx(lnd_boro_u %>% mutate_if(is.numeric, round, 4), 'analysis/london_boroughs_uniqueness.xlsx')
```

### Analyse and viz uniqueness

Based on https://mharinga.github.io/choropleth.html (accessed in May 2022).

```{r}
# bar chart
lnd_boro_u %>% select(sum_dists_class) %>% plot()
# add geometries
lnd_boro_u_geo = merge(lnd_boro %>% select(id, geometry), lnd_boro_u)
# choropleth
...
tm_shape(lnd_boro_u_geo) +
  tm_polygons('sum_dists_class', #"sum_dists_class",sum_dists_z_p
        id = "name",
        palette = "viridis",
        #style = "fisher", 
        title = "Uniqueness of London boroughs", 
        legend.hist = T,
        alpha = .5)
```
...
```{r}
wb_geo = read_sf("data/world_bank/indicators_by_year/world_bank_indicators_countries-2019.geojson") %>% filter(!is.na(iso3c) & sp.pop.totl >= 1e6)
names(wb_geo)
nrow(wb_geo)
...
) %>% select(iso_a2, iso_a3, name, region_un, population, pop_growth, gdp,
...
```{r}
# normalise and scale
wb_trans_tib = wb_tib %>% mutate(gdp = log10(gdp+1), population = log10(population+1))
...
meth = 'manhattan'
#meth = 'euclidean'
wb_uniq = dist_uniq(wb_trans_tib, meth, impute_na = T)
...
```{r}
# correlations
wb_uniq %>% select_if(is.numeric) %>% correlate(method='kendall') %>% stretch() %>% filter(x=='sum_dists') %>% arrange(-r)
...
for (col in c('sum_dists','sum_dists_z','sum_dists_class')){
  plot(wb_uniq_geo[col], border=NA)
}

#tm_shape(wb_uniq_geo) + tm_polygons("sum_dists_z", legend.hist = TRUE)
```
...
eu_geo = eu_geo %>% select(NUTS_ID, CNTR_CODE, NUTS_NAME, region_population, life_expectancy_t, region_gdp, unemployment_rate_t)
...
```{r}
method = 'euclidean'
eu_uniq = dist_uniq(eu_trans_geo %>% st_drop_geometry(), method)
...
for (col in c('sum_dists','sum_dists_z','sum_dists_class')){
  plot(eu_uniq_geo[col], border=NA)
}
```
End of notebook

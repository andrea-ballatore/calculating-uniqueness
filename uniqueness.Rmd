...
for (cols in list(c('distrib','row_n'), c('distrib','col_n'), c('distrib'),
  ...
  res_stats_tib = res_tib %>% 
    ...
    dplyr::summarise(cols = cols_str, 
      n = n(), 
      mn_dist_mean = mean(mean),
      mn_dist_med = mean(med),
...
summary_res %>% filter(cols == 'distrib') %>% dplyr::select(distrib, n, mn_vcommon, mn_common, mn_inbet, mn_rare, mn_vrare)

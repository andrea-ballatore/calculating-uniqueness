...
pacman::p_load(tidyverse, moments, knitr, corrr, entropy, pastecs, coop, sf, 
               outliers, mvoutlier, mahalanobis)
...
```{r}
# use mvoutlier
indf = as.data.frame(gen_random_matrix('2groups', 20, 5))
...
Define uniqueness calculations
...
View(res)
openxlsx::write.xlsx(res, 'analysis/high_dimensional_experiment.xlsx')
TODO

summary_res = tibble()
for (vars in list(c('distrib','dist_method'))){
     #c('distrib','dist_method'), c('distrib','dist_method','data_scale'))){
  print(vars)
  summary_stats_row = res %>% group_by_('distrib','dist_method','data_scale') %>% summarize(
    mn_med = mean(med),
    mn_max = mean(max),
    mn_skew = mean(skewness),
    mn_shap_w = mean(shapiro_w)) %>% 
    mutate(across(where(is.numeric), round, 3)) #%>% 
    #add_column(vars=paste0(vars,collapse = ' '), .before = 'distrib')
  #TODO: NOT WORKING
  View(summary_stats_row)
